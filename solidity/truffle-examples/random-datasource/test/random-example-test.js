...
const { waitForEvent, getInfuraKey } = require('./utils')
...
contract('Random Example Tests', async accounts => {
  ...
  before(async () => (
    ...
    { methods } = contract,
    { events } = new web3Socket.eth.Contract(contract._jsonInterface, contract._address)
  ))

  it('Callback should have logged a new Random Number event', async () => {
